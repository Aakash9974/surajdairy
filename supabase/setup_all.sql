...
-- ---- Edit / delete a saved sale (atomic, keeps stock correct) -------------
...
begin
  update public.products p set stock_count = p.stock_count + x.qty
  from (select si.product_id as pid, sum(si.qty) as qty from public.sale_items si
        where si.sale_id = p_sale_id and si.product_id is not null group by si.product_id) x
  where p.id = x.pid and p.track_stock and p.stock_count is not null;

  select coalesce(sum((it->>'qty')::numeric * (it->>'unit_price')::numeric), 0)
    into v_total from jsonb_array_elements(p_items) it;

  update public.sales set customer_id = p_customer_id, total_amount = v_total,
         paid_amount = coalesce(p_paid_amount, 0), payment_mode = p_payment_mode, note = p_note
   where id = p_sale_id;
  ...
  from (select (it->>'product_id')::uuid as pid, sum((it->>'qty')::numeric) as qty
        from jsonb_array_elements(p_items) it where nullif(it->>'product_id','') is not null group by 1) y
...
begin
  update public.products p set stock_count = p.stock_count + x.qty
  from (select product_id pid, sum(qty) qty from public.sale_items
        where sale_id = p_sale_id and product_id is not null group by product_id) x
  where p.id = x.pid and p.track_stock and p.stock_count is not null;
